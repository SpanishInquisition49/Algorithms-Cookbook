...
const actionsMap = {
  ...
  [ACTIONS.SELECTEDINDEX]: (action, members) => {
    ...
    if (oldI != null) members[oldI].setColor(arrayColor);
    selectedIndex = newI;
  },
...
const markSorted = (a, onAction) => {
  for (i = 0; i < a.length; i++) onAction({ type: ACTIONS.SORT, data: i });
...
const bogoSort = (items, onAction) => {
  ...
  while (!isSorted) {
    ...
    for (let i = 0; i < items.length - 1 && isSorted; i++)
      if (items[i + 1] < items[i]) isSorted = false;
  }
};
...
const buildMaxHeap = (a, len, onAction) => {
  for (let i = len; i >= 0; i--) maxHeapify(a, i, len, onAction);
  return a;
};
...
const reheapificationDownward = (h, length, onAction) => {
  ...
  for (let i = length; i >= 0; i--) maxHeapify(h, i, length, onAction);
};

const heapSort = (a, onAction) => {
  console.log(a);
  if (!isHeap(a)) buildMaxHeap(a, a.length - 1, onAction);
...
const isHeap = (h) => {
  for (let i = 0; i < Math.floor(h.length / 2); i++) {
    let c = childs(i);
    if (h[i] < h[c.sx] || h[i] < h[c.dx]) return false;
  }
  return true;
};
...
function ArrayMember(x, y, width, height, color = arrayColor) {
  ...
  this.checkSorted = () => {
    for (let i = 0; i < this.arrayMembers.length; i++)
      if (!this.arrayMembers[i].isSorted) return Premise.resolve(false);
...
const arrayMembers = randomArray.map((v, i) => {
  return new ArrayMember(
    getProportionalWidth(randomArray.length) * i,
    canvas.height - getProportionalHeigth(v, ELEMENTS),
    getProportionalWidth(randomArray.length) - 0.5,
    getProportionalHeigth(v, randomArray.length)
  );
});

const countingSort = (a, onAction) => {
  let max = a[0];
  for (let i = 1; i < a.length; i++) max = a[max] < a[i] ? a[i] : a[max];
  let counter = [];
...
const binarySearch = (a, v, onAction, l = 0, r = a.length - 1) => {
  ...
  if (a[half] == v) return half;
  if (a[half] > v) return binarySearch(a, v, onAction, l, half);
  else return binarySearch(a, v, onAction, half + 1, r);
};
...
const myAction = (action) => {
  ...
  setTimeout(() => {
    ...
    arrayMembers.forEach((m, i) =>
      i == selectedIndex ? m.setColor(swapColor) : m.resetColor()
    );
  }, ticks * SPEED);
};
...
const checkSorted = async (a) => {
  for (let i = 1; i < a.length; i++)
    if (a[i] < a[i - 1]) return await Promise.resolve(false);
...
const runSort = (f) => {
  ...
  if (res != "SEARCH") markSorted(randomArray, myAction);
};
