...
class Tree {
  ...
  static map(t, f) {
    t.value = f(t.value);
    if (t.childs != undefined) for (let c of t.childs) this.map(c, f);
  }

  static reduce(t, f) {
    let res = f(t.value);
    if (t.childs != undefined)
      for (let c in t.childs) res += this.reduce(t.childs[c], f);
    return res;
  }
  ...
  static max(t) {
    ...
    let max = t.value;
    if (t.childs != undefined)
      for (let c of t.childs) max = Math.max(max, this.max(c));
    return max;
  }
  ...
  static min(t) {
    ...
    let min = t.value;
    if (t.childs != undefined)
      for (let c of t.childs) min = Math.min(min, this.min(c));
    return min;
  }
  ...
  static search(t, v) {
    ...
    if (t.childs != undefined)
      for (c of t.childs) if (this.search(c, v)) return true;
    return false;
  }

  static pathFromRootToValue(t, v, path = [new PathSegment("Root", t.value)]) {
    if (t.childs == undefined) return t.value == v ? path : t.value;
    if (t.value == v) return path;
  ...
  static pathFromXtoY(t, x, y) {
    ...
    if (px == undefined || py == undefined) return path;
    let node = {};
    for (let i = 0; i < px.length; i++) {
      if (
        i < py.length &&
        px[i].nodeIndex == py[i].nodeIndex &&
        px[i].value == py[i].value
      ) {
